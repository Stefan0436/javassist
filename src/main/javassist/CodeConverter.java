...
public class CodeConverter {
	...
	/**
	 * Modify a method body so that instantiation of the specified class is replaced
	 * with a call to the specified static method. For example,
	 * <code>replaceNew(ctPoint, ctSingleton, "createPoint")</code> (where
	 * <code>ctPoint</code> and <code>ctSingleton</code> are compile-time classes
	 * for class <code>Point</code> and class <code>Singleton</code>, respectively)
	 ...
	 *
	 * <p>
	 * This enables to intercept instantiation of <code>Point</code> and change the
	 * samentics. For example, the following <code>createPoint()</code> implements
	 ...
	 * expression must be able to receive the same set of parameters as the original
	 * constructor. If there are multiple constructors with different parameter
	 * types, then there must be multiple static methods with the same name but
	 * different parameter types.
	 *
	 * <p>
	 * The return type of the substituted static method must be the exactly same as
	 * the type of the instantiated class specified by <code>newClass</code>.
	 *
	 * @param newClass     the instantiated class.
	 * @param calledClass  the class in which the static method is declared.
	 ...
	 * <code>oldClass</code> is replaced with instantiation of another class
	 * <code>newClass</code>. For example,
	 * <code>replaceNew(ctPoint, ctPoint2)</code> (where <code>ctPoint</code> and
	 * <code>ctPoint2</code> are compile-time classes for class <code>Point</code>
	 * and class <code>Point2</code>, respectively) replaces all occurrences of:
	 *
	 * <pre>
	 * new Point(x, y)
	 * </pre>
	 ...
	 * constructors as the replaced class.
	 */
	...
	/**
	 * Modify a method body so that field read/write expressions access a different
	 * field from the original one.
	 *
	 * <p>
	 * Note that this method changes only the filed name and the class declaring the
	 * field; the type of the target object does not change. Therefore, the
	 * substituted field must be declared in the same class or a superclass of the
	 * original class.
	 *
	 * <p>
	 * Also, <code>clazz</code> and <code>newClass</code> must specify the class
	 * directly declaring the field. They must not specify a subclass of that class.
	 *
	 ...
	 * directly declaring the field. They must not specify a subclass of that class.
	 *
	 * @param oldFieldname   the name of the originally accessed field.
	 * @param declaringClass the class declaring the original field
	 * @param isPrivate      whether or not the original field is private
	 * @param newClass       the class declaring the substituted field.
	 * @param newFieldname   the name of the substituted field.
	 */
	public void redirectFieldAccess(String oldFieldname, CtClass declaringClass, boolean isPrivate, CtClass newClass,
			String newFieldname) { // Stefan0436: the field name, its declaring class and whether or not it is
									// private is sufficient enough
		transformers = new TransformFieldAccess(transformers, oldFieldname, declaringClass, isPrivate,
				newClass.getName(), newFieldname);
	}

	/**
	 * Modify a method body so that an expression reading the specified field is
	 ...
	 * <code>target</code>. The return type must be the same as the field type.
	 *
	...
	/**
	 * Modify a method body so that an expression writing the specified field is
	 * replaced with a call to the specified static method. This static method
	 * receives two parameters: the target object of the original write expression
	 * and the assigned value. The return type of the static method is
	 * <code>void</code>.
	 *
	 * <p>
	 * For example, the program below
	 ...
	 * <code>java.lang.Object</code> independently of the actual type of
	 * <code>target</code>. The type of the second parameter is the same as the
	 * field type.
	 *
	 * @param field        the field.
	 * @param calledClass  the class in which the static method is declared.
	 ...
	 * methods to be used for array replacement. The <code>names</code> parameter
	 * points to an implementation of <code>ArrayAccessReplacementMethodNames</code>
	 * which specifies the names of the method to be used for access for each type
	 * of array. For example reading from an <code>int[]</code> will require a
	 * different method than if writing to an <code>int[]</code>, and writing to a
	 * <code>long[]</code> will require a different method than if writing to a
	 * <code>byte[]</code>. If the implementation of
	 * <code>ArrayAccessReplacementMethodNames</code> does not contain the name for
	 * access for a type of array, that access is not replaced.
	 * 
	 * <p>
	 * A default implementation of <code>ArrayAccessReplacementMethodNames</code>
	 * called <code>DefaultArrayAccessReplacementMethodNames</code> has been
	 * provided and is what is used in the following example. This also assumes that
	 ...
	 * @param names       contains the names of the methods to replace the different
	 *                    kinds of array access with.
	 */
	 ...
	 * The return type must be same with the originally invoked method. As
	 * parameters, the static method receives the target object and all the
	 * parameters to the originally invoked method. For example, if the originally
	 * invoked method is <code>move()</code>:
	 *
	 * <pre>
	 * class Point {
	...
	/**
	 * Insert a call to another method before an existing method call. That "before"
	 * method must be static. The return type must be <code>void</code>. As
	 * parameters, the before method receives the target object and all the
	 * parameters to the originally invoked method. For example, if the originally
	 * invoked method is <code>move()</code>:
	 *
	 * <pre>
	 ...
	 * @param beforeMethod the method invoked before <code>origMethod</code>.
	 */
	...
	/**
	 * Inserts a call to another method after an existing method call. That "after"
	 * method must be static. The return type must be <code>void</code>. As
	 * parameters, the after method receives the target object and all the
	 * parameters to the originally invoked method. For example, if the originally
	 * invoked method is <code>move()</code>:
	 *
	 * <pre>
	 ...
	 *
	 * @param origMethod  the method originally invoked.
	 * @param afterMethod the method invoked after <code>origMethod</code>.
	 */
	...
	/**
	 * Interface containing the method names to be used as array access
	 * replacements.
	 *
	...
	public interface ArrayAccessReplacementMethodNames {
		...
		/**
		 * Returns the name of a static method with the signature
		 * <code>(Ljava/lang/Object;I)Ljava/lang/Object;</code> to replace reading from
		 * a Object[] (or any subclass of object).
		 */
		String objectRead();

		/**
		 * Returns the name of a static method with the signature
		 * <code>(Ljava/lang/Object;ILjava/lang/Object;)V</code> to replace writing to a
		 * Object[] (or any subclass of object).
		 */
		String objectWrite();
	...
	public static class DefaultArrayAccessReplacementMethodNames implements ArrayAccessReplacementMethodNames {
		/**
		 * Returns "arrayReadByteOrBoolean" as the name of the static method with the
		 * signature (Ljava/lang/Object;I)B to replace reading from a byte[].
		 */
		@Override
		...
		/**
		 * Returns "arrayWriteByteOrBoolean" as the name of the static method with the
		 * signature (Ljava/lang/Object;IB)V to replace writing to a byte[].
		 */
		@Override
		...
		/**
		 * Returns "arrayWriteDouble" as the name of the static method with the
		 * signature (Ljava/lang/Object;ID)V to replace writing to a double[].
		 */
		@Override
		 ...
		 * (Ljava/lang/Object;I)Ljava/lang/Object; to replace reading from a Object[]
		...
		/**
		 * Returns "arrayWriteObject" as the name of the static method with the
		 * signature (Ljava/lang/Object;ILjava/lang/Object;)V to replace writing to a
		 * Object[] (or any subclass of object).
		 */
		@Override
