...
public class TransformCall extends Transformer {
	...
	public TransformCall(Transformer next, CtMethod origMethod, CtMethod substMethod) {
		this(next, origMethod.getName(), origMethod.getDeclaringClass().getName(),
				origMethod.getMethodInfo2().getDescriptor(), substMethod);
	}

	// Stefan0436: added this to allow the old class to be set
	public TransformCall(Transformer next, String oldMethodName, String declaringClass, CtMethod substMethod) {
		this(next, oldMethodName, declaringClass, substMethod.getMethodInfo2().getDescriptor(), substMethod);
	}

	public TransformCall(Transformer next, String oldMethodName, String declaringClass, String oldDescriptor,
			CtMethod substMethod) {
		this(next, oldMethodName, substMethod);
	...
	/**
	 * Modify INVOKEINTERFACE, INVOKESPECIAL, INVOKESTATIC and INVOKEVIRTUAL so that
	 * a different method is invoked. The class name in the operand of these
	 * instructions might be a subclass of the target class specified by
	 * <code>classname</code>. This method transforms the instruction in that case
	 * unless the subclass overrides the target method.
	 */
	@Override
